...
## Project Structure
- `src/app.ts` MUST build and export the Fastify app and MUST NOT call `listen()`.
- `src/server.ts` MUST be the only file that calls `listen()`.
- `src/plugins/` MUST contain Fastify plugins.
- `src/routes/` MUST contain route registrations grouped by resource.
- `src/schemas/` MUST contain shared TypeBox schemas.
- `src/db/` MUST contain the Drizzle client, schema, and migrations.
...
## Database
- Drizzle schema MUST live in `src/db/schema.ts`.
- Migrations MUST live in `src/db/migrations/`.
